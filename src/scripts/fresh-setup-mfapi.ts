...
const DATABASE_URL = process.env.RAILWAY_DATABASE_URL || 'postgresql://postgres:zZGzhpULOgKqXvnnutWEjCengioSheMD@turntable.proxy.rlwy.net:19665/railway';
...
async function setupFreshDatabase() {
  console.log('🚀 Setting up fresh database with MFApi...\n');
  ...
  try {
    ...
    console.log('📥 Step 2: Fetching mutual fund list from MFApi...');
    
    // Get list of all schemes from MFApi
    const response = await axios.get('https://api.mfapi.in/mf', {
      timeout: 30000
    });

    const schemes = response.data;
    console.log(`✅ Found ${schemes.length} mutual funds\n`);

    console.log('💾 Step 3: Inserting funds into database...');
    ...
    for (let i = 0; i < schemes.length; i += batchSize) {
      const hasSpace = await checkDatabaseSpace();
      if (!hasSpace) {
        console.error(`\n❌ Database 90% full - stopped at ${insertedCount}/${schemes.length} funds`);
        break;
      }
      ...
      batch.forEach((scheme: any, idx: number) => {
        const baseIdx = idx * 3;
      ...
      await pool.query(`
        INSERT INTO funds (scheme_code, scheme_name, is_active)
      ...
      console.log(`   ✅ Inserted ${insertedCount}/${schemes.length} funds`);
    }

    console.log('\n' + '='.repeat(60));
    console.log('✨ FRESH DATABASE SETUP COMPLETE\n');
    console.log(`   📊 Total Funds: ${insertedCount}`);
    console.log(`   ✅ Schema: Created`);
    console.log(`   ✅ Funds: Populated`);
